...
/**
 * 문제 설명
 * 코딩테스트를 준비하는 머쓱이는 프로그래머스에서 문제를 풀고 나중에 다시 코드를 보면서 공부하려고 작성한 코드를
 * 컴퓨터 바탕화면에 아무 위치에나 저장해 둡니다. 저장한 코드가 많아지면서 머쓱이는 본인의 컴퓨터 바탕화면이
 * 너무 지저분하다고 생각했습니다. 프로그래머스에서 작성했던 코드는 그 문제에 가서 다시 볼 수 있기 때문에
 ...
 * 마우스 왼쪽 버튼을 떼는 행동입니다. 이때, "점 S에서 점 E로 드래그한다"고 표현하고 점 S와 점 E를
 ...
 * eg1-2.png
 * (0, 0)에서 (3, 5)로 드래그해도 모든 파일을 선택할 수 있지만 이때 드래그 한 거리는 (3 - 0) + (5 - 0) = 8이고
 * 이전의 방법보다 거리가 늘어납니다.
 *
...
public class Solution73 {
    ...
    public int[] solution(String[] wallpaper) {
        ...
        answerList.put("top", wallpaper.length);
        answerList.put("left", wallpaper[0].length());
        answerList.put("bottom", 0);
        answerList.put("right", 0);

        for(int i=0; i<wallpaper.length; i++){
            String paper = wallpaper[i];
            for(int j=0; j<paper.length(); j++){
                if(paper.charAt(j) == '#'){
                    if(i < answerList.get("top"))
                        answerList.put("top", i);
                    if(i >= answerList.get("bottom"))
                        answerList.put("bottom", i+1);
                    if(j < answerList.get("left"))
                        answerList.put("left", j);
                    if(j >= answerList.get("right"))
                        answerList.put("right", j+1);
                }
            }
        }
        answer[0] = answerList.get("top");
        answer[1] = answerList.get("left");
        answer[2] = answerList.get("bottom");
        answer[3] = answerList.get("right");
        return answer;
    }
