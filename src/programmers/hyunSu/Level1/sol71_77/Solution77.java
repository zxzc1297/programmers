...
public class Solution77 {
    ...
    public int[] solution(String[] id_list, String[] report, int k) {
        ...
        HashMap<String, Integer> cntList = new HashMap<>();
        ...
        for(String rep : report){
            ...
            String reported = splits[1];

            ArrayList<String> curList = reportList.get(reporter);
            if(!curList.contains(reported)){
                curList.add(reported);
                int curCnt = cntList.get(reported);
                cntList.put(reported, curCnt+ 1);
            }
            if(cntList.get(reported) >= k){
                if(!stopped.contains(reported))
                    stopped.add(reported);
            }

            reportList.put(reporter, curList);
        }

        for(int i=0; i< id_list.length; i++){
            int cnt = 0;
            ArrayList<String> list = reportList.get(id_list[i]);
            for(String person : list){
                if(stopped.contains(person))
                    cnt++;
            }
            answer[i] = cnt;
        }
