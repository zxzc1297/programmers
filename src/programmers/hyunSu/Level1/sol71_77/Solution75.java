 ...
 * 문자열 배열 callings가 매개변수로 주어질 때, 경주가 끝났을 때 선수들의 이름을 1등부터 등수 순서대로 배열에 담아
...
public class Solution75 {
    ...
    public String[] solution(String[] players, String[] callings) {
        ...
        for(int j=0; j<callings.length; j++){
            int callPlayer = playerList.get(callings[j]);
            String changePlayer = players[callPlayer-1];
            players[callPlayer-1] = callings[j];
            players[callPlayer] = changePlayer;
            playerList.put(callings[j], callPlayer-1);
            playerList.put(changePlayer, callPlayer);
        }
