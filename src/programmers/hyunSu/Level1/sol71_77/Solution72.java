...
public class Solution72 {
    ...
    public String solution(String[] survey, int[] choices) {
        String answer = "";
        HashMap<String, Integer> mbtiList = new HashMap<>();
        mbtiList.put("R", 0);
        mbtiList.put("T", 0);
        mbtiList.put("C", 0);
        mbtiList.put("F", 0);
        ...
        for(int i=0; i<survey.length; i++){
            ...
            if(choices[i] > 4)
                mbtiList.put(survey[i].substring(1,2), mbtiList.get(survey[i].substring(1,2)) + choices[i]-4);
            else if(choices[i] < 4)
                mbtiList.put(survey[i].substring(0,1), mbtiList.get(survey[i].substring(0,1)) + 4-choices[i]);
        }

        if(mbtiList.get("R") >= mbtiList.get("T"))
            answer += "R";
        else
            answer += "T";

        if(mbtiList.get("C") >= mbtiList.get("F"))
            answer += "C";
        else
            answer += "F";

        if(mbtiList.get("J") >= mbtiList.get("M"))
            answer += "J";
        else
            answer += "M";

        if(mbtiList.get("A") >= mbtiList.get("N"))
            answer += "A";
        else
            answer += "N";
