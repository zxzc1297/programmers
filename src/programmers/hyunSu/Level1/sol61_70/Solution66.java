 ...
 * 담긴 문자열 배열 targets가 주어질 때, 각 문자열을 작성하기 위해 키를 최소 몇 번씩 눌러야 하는지
...
public class Solution66 {
    ...
    public int[] solution(String[] keymap, String[] targets) {
        ...
        HashMap<Character,Integer> keyMapList = new HashMap<>();

        for(int i=0; i<keymap.length; i++){
            char[] chars = keymap[i].toCharArray();
            for(int j=0; j<chars.length; j++){
                if(keyMapList.containsKey(chars[j])){
                    if(keyMapList.get(chars[j]) > j+1){
                        keyMapList.put(chars[j], j+1);
                    }
                }else{
                    keyMapList.put(chars[j], j+1);
                }
        ...
        for(int k=0; k<targets.length; k++){
            int result = 0;
            char[] chars2 = targets[k].toCharArray();
            for(char cha2 : chars2){
                if(keyMapList.containsKey(cha2)){
                    result += keyMapList.get(cha2);
                }else{
                    result = -1;
                    break;
                }
