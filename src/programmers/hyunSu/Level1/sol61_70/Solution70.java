...
public class Solution70 {
    ...
    public String solution(String new_id) {
        ...
        for(int i=0; i<chars.length; i++){
            ...
            if((sb.toString().equals("") || sb.toString() == null) && add == '.'){
                continue;
            }
            sb.append(add);
        }
        ...
        if(chars[chars.length-1] == '.')
            answer = sb.substring(0, chars.length -1);
        ...
        if(answer.length()<=2){
            while(answer.length()!=3)
                answer += answer.toString().substring(answer.length()-1, answer.length());
            return answer;
        }
    ...
    public boolean task2(char c){
        if(!(c >=97 && c<123) && (c != '-') && (c!='_') && (c!='.') && !(c >=48 && c<=57))
            return false;
